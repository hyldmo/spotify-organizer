...
export function getDeduplicateErrors (mode: OperationMode, selectedPlaylists: Playlist[], secondPlaylist: Playlist | null, user: User): string | null {
	...
	switch (mode) {
		...
		case OperationMode.PullTracks:
			if (secondPlaylist === null)
				return 'You must select a second playlist'
			break
	}
