	...
	return playback?.currently_playing_type == 'track' && song ? (
		<div className="grid grid-rows-[1fr,1fr,auto] grid-cols-[auto,auto,auto,1fr,6fr,1fr,auto] px-2 py-3 gap-x-4 items-center">
			...
			<div className="col-start-7 row-span-2 flex items-center space-x-2">
				<button
					className={settings?.watchSkips ? 'text-green-600' : 'text-gray-700'}
					title="Watch Skips"
					onClick={_ => dispatch(Actions.updateSettings(!settings?.watchSkips, 'watchSkips'))}
				>
					<FontAwesomeIcon icon="fast-forward" />
				</button>
				<input type="range" min={0} max={100} step="1" readOnly value={playback.device.volume_percent || 100} />
